...
def add_data_redis(coin_type, add_data):
    ...
    if __init__.推送_通道_深度图 != '':
        redis_conn.REDIS.publish(__init__.推送_通道_深度图, res_add_data)
    if __init__.推送_合约_深度图 != '':
        redis_conn.REDIS.set(__init__.推送_合约_深度图 % (coin_type), res_add_data)
